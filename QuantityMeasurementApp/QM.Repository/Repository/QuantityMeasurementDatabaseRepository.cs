    ...
    public class QuantityMeasurementDatabaseRepository : IQuantityMeasurementRepository
    {
        ...
        public List<QuantityMeasurementEntity> GetAll(string? userId = null)
        {
            try
            {
                ...
                var measurements = query.OrderByDescending(m => m.Timestamp).ToList();
        ...
        public List<QuantityMeasurementEntity> GetByOperationType(string operationType, string? userId = null)
        {
            try
            {
                var query = _context.Measurements.Where(m => m.OperationType.ToUpper() == operationType.ToUpper());
                if (!string.IsNullOrEmpty(userId))
                {
                    query = query.Where(m => m.UserId == userId);
                }
        ...
        public List<QuantityMeasurementEntity> GetErroredMeasurements(string? userId = null)
        {
            try
            {
                var query = _context.Measurements.Where(m => m.HasError == true);
                if (!string.IsNullOrEmpty(userId))
                {
                    query = query.Where(m => m.UserId == userId);
                }
        ...
        public int GetCountByOperationType(string operationType, string? userId = null)
        {
            try
            {
                ...
                return query.Count(m => m.OperationType.ToUpper() == operationType.ToUpper());
            }
            catch (Exception ex)
            {
